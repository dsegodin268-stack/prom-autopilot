...
# 1) повна позиція в наявності        -> рівень 1 -> Export
# 2) без фото, під замовлення 15 днів -> рівень 3 -> Staging «чекає фото»
# 3) фото є, характеристик обмаль     -> рівень 2 -> Staging «на перевірку»
FEED_ROWS = [
