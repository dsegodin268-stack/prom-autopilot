# validator.py — перевірка картки товару за ПРАВИЛА_PROM.md
# Token-free, без API-викликів. Повертає прапорці для ЗВІТу (не пише в Prom).
# Рівні: CRITICAL — Prom відхилить при імпорті; WARN — пройде, але якість/комплаєнс під питанням.
import re

CRITICAL = "CRITICAL"
WARN = "WARN"

# [Prom] закритий список одиниць виміру (помилки імпорту /360004960997)
PROM_UNITS = {
...
def validate_parts_description(desc):
    """Перевірка шаблону ЗАПЧАСТИНИ (розд. 3a): сумісність + OEM/замінники + CTA."""
    out = []
    low = (desc or "").lower()
...
def validate_characteristics(chars):
    """chars: список трійок (назва, одиниця, значення)."""
    out = []
    chars = chars or []
    if len(chars) < 2:
...
def validate_card(card, is_part=True):
    """card: dict з ключами name, description, chars, images, price, presence, group_id, product_id.
...
def summarize(flags):
    """Короткий рядок для колонки ЗВІТу."""
    if not flags:
        return "OK"
