...
2026-07-28 — додано дві сходинки (Kilo Gateway, Vercel AI Gateway), розділено
чергу аудиту й правки, виправлено паузи, дописано жорсткі правила мови.
...
СКІЛЬКИ ЦЕ НАСПРАВДІ ЗАПИТІВ. Тут довго стояла цифра «3913 позицій = 3913
запитів = одна доба». Вона була помилкова: на одну позицію йде ДО ТРЬОХ
викликів — створення (enrich_facts), перевірка (audit_card) і правка за
зауваженнями (repair_fields). Повний перезалив каталогу — це ~9800 запитів
і ~29 млн токенів, а не 3913. Тому:
  * добові стелі нижче рахуйте на ТРИ, а не на одну;
  * аудит і правка мають СВОЮ чергу (AI_PROVIDERS_AUDIT / AI_PROVIDERS_FIX),
    щоб дешева перевірка не з'їдала квоту найкращої моделі;
  * AI_AUDIT=0 і AI_FIX=0 миттєво повертають витрату до 1 запиту на позицію.

Порядок задає AI_PROVIDERS (через кому). За замовчуванням — ORDER нижче:
спершу найкраща якість, далі найбільша ємність, платні (Vercel, Anthropic) —
в кінці, щоб гроші витрачались тільки коли безкоштовне вичерпано.
...
# ---------------------------------------------------------------- промпти
# ЖОРСТКІ ПРАВИЛА МОВИ. Причина конкретна, а не «про всяк випадок»: прогін
# 27.07 віддав картку, у якої і Назва_позиції, і Назва_позиції_укр були
# російські («Пыльник амортизатора (переднего) BMW 3 (F30)...»). JSON був
# формально правильний, а української картки на Prom не було зовсім. Модель
# сама тягне на російську, бо факти від постачальника російські, — тому
# вимогу треба писати словами, а не сподіватись на здогад.
_LANG_RULES = (
    "МОВА. Поля з суфіксом _ru — РОСІЙСЬКОЮ, поля з суфіксом _ua — УКРАЇНСЬКОЮ. "
    "Українське поле не може бути копією російського і не може бути російським "
    "словом, записаним українськими літерами. Перекладай терміном, а не калькою: "
    "пыльник->пильник, подшипник->підшипник, тормозной диск->гальмівний диск, "
    "тормозные колодки->гальмівні колодки, масляный фильтр->масляний фільтр, "
    "воздушный фильтр->повітряний фільтр, салонный фильтр->фільтр салону, "
    "топливный фильтр->паливний фільтр, ремень->ремінь, сцепление->зчеплення, "
    "рулевая рейка->кермова рейка, ступица->маточина, отбойник->відбійник, "
    "стойка стабилизатора->стійка стабілізатора, рычаг->важіль, "
    "выхлопной коллектор->випускний колектор, впускной->впускний, "
    "передний->передній, задний->задній, левый->лівий, правый->правий, "
    "верхний->верхній, нижний->нижній, наружный->зовнішній, внутренний->внутрішній. "
    "Марка, кузов, двигун і каталожний номер лишаються латиницею в ОБОХ мовах: "
    "BMW, F30, N20, 11427953129. Якщо українського терміна не знаєш — опиши "
    "деталь простими українськими словами, але російське слово НЕ лишай.\n"
)

# ЗНАХОДЖУВАНІСТЬ. Вимога власника дослівно: кожну позицію мають знаходити
# одразу — за повною назвою, АБО за каталожним номером, АБО просто за авто,
# на яке вона підходить. Це три РІЗНІ шляхи пошуку, і кожен треба закрити
# ключовими словами явно, інакше картка знаходиться лише тим, хто вже знає,
# що шукає.
_FIND_RULES = (
    "ЗНАХОДЖУВАНІСТЬ. Картку мають знайти ТРЬОМА різними шляхами, і кожен треба "
    "закрити окремо: 1) повною назвою деталі ('пильник амортизатора переднього BMW'); "
    "2) каталожним номером — і самим номером окремим елементом, і у фразі разом із "
    "типом деталі; 3) самим автомобілем, без назви деталі ('BMW F30 2.0', "
    "'запчастини BMW F30'). Якщо хоч один із трьох шляхів не закритий — список "
    "ключових слів неповний.\n"
    "ЗАБОРОНЕНІ СЛОВА в назві, мета-полях і ключових словах: купити, купить, продаж, "
    "ціна, цена, оптом, недорого, дешево, знижка, акція, доставка, гарантія, "
    "склад, магазин, а також назва самого магазину і будь-які міста та регіони "
    "(Київ, Україна, Одеса, Харків...). Prom ріже за них релевантність, Google "
    "вважає це спамом.\n"
)

# ЧУЖА ТЕРИТОРІЯ. ПРАВИЛА_PROM §8: ШІ не має РУК до технічних полів. Фізично
# це вже так (merge_ai уміє писати рівно в 10 текстових ключів), але модель про
# це не знає і щоразу пробує «допомогти», підсунувши групу чи вагу. Тоді ці
# значення просто мовчки викидаються — і виглядає так, ніби ШІ їх врахував.
_SCOPE_RULES = (
    "ЧУЖА ТЕРИТОРІЯ. Ти НЕ призначаєш і НЕ згадуєш у відповіді: ціну, собівартість, "
    "валюту, наявність, кількість, номер і назву групи, підрозділ, виробника, GTIN, "
    "вагу, адреси фото. Це рахує код за канонічною таблицею; будь-яке твоє значення "
    "тут буде відкинуте, а місце у відповіді втрачене.\n"
    "ФОРМАТ ВІДПОВІДІ. Тільки JSON-об'єкт, без ``` і без пояснень до чи після нього. "
    "Усі 10 ключів присутні ЗАВЖДИ; keywords_ru і keywords_ua — масиви рядків, "
    "решта — рядки. Порожнім не лишай нічого: бракує фактів — пиши коротше.\n"
)
...
_COMMON_RULES = (
    ...
    "Опис мусить прямо казати, НА ЯКІ авто деталь підходить і КУДИ вона ставиться. "
    ...
    + _LANG_RULES + _FIND_RULES + _SCOPE_RULES
)
...
# Звірено 26.07.2026 з cheahjs/free-llm-api-resources, паузи перераховані 28.07.
# Паузи рахуються так, щоб не впертись у tokens/minute при ~2500 токенах на
# запит, — саме tpm, а не rpm, майже завжди є вузьким місцем.
#
...
PROVIDERS = {
    ...
    # Число 8600 нижче — стеля НА ПАПЕРІ. За 6 годин, які живе job у GitHub
    # Actions, при паузі 10 с фізично влазить ~2100 запитів, тож лічильник
    # спрацює лише в дуже довгому прогоні. Лишаємо як запобіжник, не як план.
    "gemma": ("https://generativelanguage.googleapis.com/v1beta/openai/chat/completions",
              ...
              10.0, 8600),
    # Groq: на gpt-oss-120b 1000 запитів/добу і 12000 tpm (у коді стояло 8000 —
    # звідси й зайва пауза 20 с). 12000 / 2500 = ~4.8 запиту/хв -> 13 с.
    ...
    # Пауза була 12.5 с (тобто ~5 rpm) — ми душили найшвидшого провайдера в 5 разів.
    # 60000 tpm / 2500 = 24 запити/хв -> 2.6 с з невеликим запасом.
    "cerebras": ("https://api.cerebras.ai/v1/chat/completions",
                 "CEREBRAS_API_KEY", ("gpt-oss-120b", "llama-3.1-8b"), 2.6, 400),
    ...
    "cloudflare": ("https://api.cloudflare.com/client/v4/accounts/{acct}/ai/v1/chat/completions",
                   ...
                   3.0, 200),
    # Kilo Gateway (додано 28.07): один ключ -> кілька чужих моделей із суфіксом
    # «:free». Ліміт ~200 запитів/ГОДИНУ, і рахується він ПО IP-адресі, а не по
    # ключу. Раннер GitHub Actions — спільна машина Microsoft, тобто частину цієї
    # години можуть з'їсти сторонні люди ще до нашого прогону. Тому сходинка
    # ЗАПАСНА: пауза 18 с рівно під 200/год, стеля 4000/добу як запобіжник.
    ...
    # Vercel AI Gateway (додано 28.07): НЕ безкоштовний тариф, а $5 кредитів на
    # місяць, зате без добової стелі й на нормальних платних моделях. Повний
    # перезалив каталогу (~9800 запитів, ~29 млн токенів) на flash-lite коштує
    # близько $3 — тобто кредитів вистачає на один повний прохід щомісяця.
    # Це СТРАХОВИЙ рунг: стоїть у кінці, тому вмикається лише коли безкоштовні
    # квоти вичерпані. Нема секрета — нема витрат: сходи його просто не бачать.
    # Модель задається у форматі «творець/назва».
...
ORDER = ["gemini", "gemma", "groq", "cerebras", "mistral", "nvidia",
         "scaleway", "cloudflare", "kilo", "openrouter", "cohere",
         "vercel", "github", "anthropic"]

# Черга за замовчуванням для АУДИТУ. Аудит не пише текст — він читає готову
# картку і називає проблеми. Це робота, з якою впорається маленька швидка
# модель, а доти вона йшла тією самою чергою, що й створення, і з'їдала квоту
# gemini/gemma — тобто найкращу українську ми витрачали на перевірку. Порядок
# нижче ставить попереду тих, у кого багато запитів і висока швидкість.
# Перебивається змінною AI_PROVIDERS_AUDIT; якщо в сходах немає жодного з цих
# провайдерів, автоматично впаде назад на загальний ORDER.
AUDIT_ORDER = ["cerebras", "groq", "nvidia", "mistral", "cloudflare", "kilo",
               "scaleway", "gemma", "openrouter", "cohere", "vercel",
               "github", "gemini", "anthropic"]
...
# Яка змінна оточення задає чергу для якого виду виклику. Три черги, а не одна,
# бо три виклики на позицію мають різну ціну помилки: у створенні тексту важлива
# українська (дорога модель), у перевірці — лише уважність (дешева й швидка), у
# правці — знову текст. Порожня змінна означає «як було»: беремо AI_PROVIDERS.
_QUEUE_ENV = {"main": "AI_PROVIDERS", "audit": "AI_PROVIDERS_AUDIT",
              "fix": "AI_PROVIDERS_FIX"}
_QUEUE_DEFAULT = {"audit": AUDIT_ORDER}


def _ladder(kind="main"):
    """Сходи провайдерів для конкретного виду виклику.

    Пріоритет такий: власна змінна черги (AI_PROVIDERS_AUDIT / AI_PROVIDERS_FIX)
    -> загальна AI_PROVIDERS -> вбудований порядок. Вбудований порядок для аудиту
    свій (AUDIT_ORDER), для решти — ORDER.

    Останній рядок із fallback потрібен на випадок, коли для аудиту не заведено
    ключа ЖОДНОГО з дешевих провайдерів: тоді краще проаудитувати дорогою
    моделлю, ніж не проаудитувати взагалі."""
    raw = (os.environ.get(_QUEUE_ENV.get(kind, "AI_PROVIDERS")) or "").strip()
    if not raw and kind != "main":
        raw = (os.environ.get("AI_PROVIDERS") or "").strip()
    if raw:
        names = [p.strip().lower() for p in raw.split(",") if p.strip()]
    else:
        names = _QUEUE_DEFAULT.get(kind, ORDER)
    out = [p for p in names if _ready(p)]
    if not out and kind != "main":
        out = [p for p in ORDER if _ready(p)]
    return out
...
#
# 28.07. Нижче до згенерованого промпта додається _AUDIT_HARD. Свідомо БЕЗ
# ЖОДНОГО ЧИСЛА: усі межі й далі живуть тільки в rules.py, а тут — якісні
# вимоги, яких у rules.py немає й порахувати їх формулою не можна (мова,
# три шляхи пошуку, слова-сміття). Дублювання чисел так і не з'являється.
_AUDIT_HARD = (
    "\n\nПОНАД СПИСОК ВИЩЕ — перевіряй ЗАВЖДИ ще й це:\n"
    "- «назва»: якщо українська назва написана російською, або дослівно збігається "
    "з російською, або це російське слово українськими літерами (пыльник, "
    "подшипник, тормозной) — пиши зауваження «назва: українська назва не українська»;\n"
    "- «запити»: чи закриті ТРИ шляхи пошуку — повною назвою деталі, каталожним "
    "номером і самим автомобілем без назви деталі. Якщо якогось нема — «запити: "
    "не закрито пошук за <шлях>»;\n"
    "- «назва» і «запити»: будь-яке зі слів купити/ціна/оптом/недорого/дешево/"
    "доставка/гарантія/магазин або назва міста — це зауваження;\n"
    "- «назва»: каталожний номер із пробілами або дефісами всередині;\n"
    "- «опис»: чи сказано прямо, на які авто деталь підходить і куди ставиться. "
    "Опис без цього — «опис: не видно, куди ставиться».\n"
    "Кожне зауваження пиши РІВНО у форматі «поле: чому» — інакше другий прохід "
    "його не побачить і не виправить."
)
AUDIT_SYSTEM = rules.audit_system() + _AUDIT_HARD
...
def providers_ready():
    ...
    ключа жодного з провайдерів, ladder виходив порожній, _ai_call повертав
...
def audit_providers_ready():
    """Те саме, але для черги аудиту. Потрібне окремо: сходи тепер РІЗНІ, і
    ситуація «створювати картки є чим, а перевіряти нема» тепер можлива."""
    return _ladder("audit")
