# -*- coding: utf-8 -*-
"""МОДУЛЬ «ЦІНИ КОНКУРЕНТІВ» (опційний; потребує playwright — не в requirements.txt).
Скрапер конкурентних цін з видачі Prom по OEM-артикулу (Prom — React/Apollo, треба
headless-браузер). Виключає свій магазин, бере першу позицію-конкурента + найдешевшу.
Результат читає repricing через вкладку «competitors» (зараз наповнюється вручну)."""
import csv
import re
import time
...
def _is_instock(a):
    a = (a or "").lower()
    return ("наявн" in a) or ("відправк" in a)
...
def pick_competitors(cards, self_store=SELF_STORE):
    ...
    for c in cards:
        s = (c.get("seller") or "")
        if not s or self_store.lower() in s.lower():
            continue
...
def scrape(articles, out_csv="competitors.csv", delay=2.5, headless=True):
        ...
        for art in articles:
            ...
            try:
                ...
                for c in pg.query_selector_all('[data-qaid="product_block"]'):
                    g = lambda sel: (c.query_selector(sel).inner_text().strip()
                                     if c.query_selector(sel) else None)
                    cards.append({"name": g('[data-qaid="product_name"]'),
                                  "price": g('[data-qaid="product_price"]'),
                                  "seller": g('[data-qaid="company_name"]'),
                                  "avail": g('[data-qaid="presence"]')})
    ...
    with open(out_csv, "w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=["article", "competitor_first", "competitor_min"])
        w.writeheader()
        w.writerows(rows)
